<script lang="ts">
	...
	function update() {
		...
		zone1 = Object.keys(prices[zipCode]).filter(target => prices[zipCode][target] < 18)
		zone2 = Object.keys(prices[zipCode]).filter(target => prices[zipCode][target] < 26 && prices[zipCode][target] >= 18)
		zone3 = Object.keys(prices[zipCode]).filter(target => prices[zipCode][target] >= 26)
		...
		<div class="relative mt-12 lg:mt-24 lg:grid lg:grid-cols-2 lg:gap-8 lg:items-center">
			<div class="relative">
				...
				<p class="mt-3 text-lg text-gray-500">
					...
					Du hast die Möglichkeiten die Lieferzonen zu konfigurieren, um festzulegen wohin geliefert werden darf und zu welchem Mindestbestellpreis.
				</p>
						...
						<div class="bg-white rounded-md -space-y-px">
							<label class="{ zone === 1 ? 'rounded-tl-md rounded-tr-md relative border p-4 flex items-center cursor-pointer focus:outline-none bg-red-50 border-red-200 z-10' : 'rounded-tl-md rounded-tr-md relative border p-4 flex items-center cursor-pointer focus:outline-none border-gray-200'}">
								...
								<div class="ml-3 flex flex-col flex-grow">
									<span class="{ zone === 1 ? 'block text-sm font-medium text-red-900' : 'block text-sm font-medium text-gray-900'}">
										Zone 1 für {zipCode}
									</span>
									<span class="{zone === 1 ? 'block text-sm text-red-700' : 'block text-sm text-gray-500'}">
										Beinhaltet:{#each zone1 as zip, i}{#if i !== 0}<span>,</span>{/if} {zip}{/each}
									</span>
								</div>
							...
							<label class="{ zone === 2 ? 'relative border p-4 flex items-center cursor-pointer focus:outline-none bg-red-50 border-red-200 z-10' : 'relative border p-4 flex items-center cursor-pointer focus:outline-none border-gray-200'}">
								<input type="radio" name="zone" value={2} checked={zone===2} on:change={onChange}
											 ...
											 aria-labelledby="privacy-setting-0-label" aria-describedby="privacy-setting-0-description">
								<div class="ml-3 flex flex-col flex-grow">
									<span class="{ zone === 2 ? 'block text-sm font-medium text-red-900' : 'block text-sm font-medium text-gray-900'}">
										Zone 2 für {zipCode}
									</span>
									<span class="{zone === 2 ? 'block text-sm text-red-700' : 'block text-sm text-gray-500'}">
										Beinhaltet:{#each zone2 as zip, i}{#if i !== 0}<span>,</span>{/if} {zip}{/each}
									</span>
								</div>
								<div class="flex items-center justify-center px-3 py-2 rounded-md bg-black text-white">
									<h4>18</h4>
							...
							<label class="{ zone === 3 ? 'rounded-bl-md rounded-br-md relative items-center border p-4 flex cursor-pointer focus:outline-none bg-red-50 border-red-200 z-10' : 'rounded-bl-md rounded-br-md relative items-center border p-4 flex cursor-pointer focus:outline-none border-gray-200'}">
								<input type="radio" name="zone" value={3} checked={zone===3} on:change={onChange}
											 ...
											 aria-labelledby="privacy-setting-0-label" aria-describedby="privacy-setting-0-description">
								<div class="ml-3 flex flex-col flex-grow">
									<span class="{ zone === 3 ? 'block text-sm font-medium text-red-900' : 'block text-sm font-medium text-gray-900'}">
										Zone 3 für {zipCode}
									</span>
									<span class="{zone === 3 ? 'block text-sm text-red-700' : 'block text-sm text-gray-500'}">
										Beinhaltet:{#each zone3 as zip, i}{#if i !== 0}<span>,</span>{/if} {zip}{/each}
									</span>
								</div>
								<div class="flex items-center justify-center px-3 py-2 rounded-md bg-black text-white">
									<h4>27</h4>
				...
				<div class='mt-6 md:mt-12'>
					...
					<p class="mt-3 text-lg text-gray-500">
						Die Aufteilung steuert wieviel der Lieferkosten dem Kunden transparent als Lieferkosten zuzüglich dem Bestellwert verrechnet wird.
					</p>
					<span class="relative z-0 inline-flex shadow-sm rounded-md mt-2">
						{#each splits as split, i}
						<button type="button" class:-ml-px={i !== 0} class:rounded-l-md={i === 0} class:rounded-r-md={i +1 === splits.length} class:ring-1={split === deliverySplit}
										class:z-20={split === deliverySplit}  class:ring-red-500={split === deliverySplit} class:border-red-500={split === deliverySplit} class:text-red-500={split === deliverySplit} class:text-gray-700={split !== deliverySplit}
			...
			<div class="mt-10 -mx-4 relative lg:mt-0 flex flex-col justify-center items-center" aria-hidden="true">
					...
					<div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
						<div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
							<table class="min-w-full divide-y divide-gray-200">
								...
								<tr>
									<th scope="col" class="px-6 py-3 w-16 text-xs text-left font-medium text-gray-500 uppercase tracking-wider">
										Bestellwert
									</th>
						...
						<div class='mt-2 mx-3 text-sm text-gray-600'>* Lieferkosten werden dem Kunden zusätzlich zum Bestellwert verrechnet, die Gebühr beinhaltet Zahlungsabwicklung- sowie Transportkosten welche vom Bestellwert abgezogen werden</div>
					</div>
				</div>
			</div>
		</div>
